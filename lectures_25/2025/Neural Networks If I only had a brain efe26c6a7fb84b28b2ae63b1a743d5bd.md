...
![Screenshot 2024-02-14 at 8.32.21 PM.png](Neural%20Networks%20If%20I%20only%20had%20a%20brain/Screenshot_2024-02-14_at_8.32.21_PM.png)

1. No more homework! [Project](https://www.notion.so/Project-0a46c078fb2748f8acfea388b216fddd?pvs=21) 
    
    > Fun example: 20 Questions bot https://github.com/earthtojake/20q
    > 
2. Tips for using the shell (terminal, command line)
    
    [ShellIntro.pdf](Neural%20Networks%20If%20I%20only%20had%20a%20brain/ShellIntro.pdf)
    

# **Neural networks overview**

![70wnk5kfr5hc1.jpeg](Neural%20Networks%20If%20I%20only%20had%20a%20brain/70wnk5kfr5hc1.jpeg)

## Biological inspiration

![Untitled](Neural%20Networks%20If%20I%20only%20had%20a%20brain/Untitled.png)
...
![Untitled](Neural%20Networks%20If%20I%20only%20had%20a%20brain/Untitled%201.png)

![Untitled](Neural%20Networks%20If%20I%20only%20had%20a%20brain/Untitled%202.png)

![Untitled](Neural%20Networks%20If%20I%20only%20had%20a%20brain/Untitled%203.png)
...
![Screenshot 2024-02-26 at 1.02.39 PM.png](Neural%20Networks%20If%20I%20only%20had%20a%20brain/Screenshot_2024-02-26_at_1.02.39_PM.png)

![ann.png](Neural%20Networks%20If%20I%20only%20had%20a%20brain/ann.png)

![nn_overview.png](Neural%20Networks%20If%20I%20only%20had%20a%20brain/nn_overview.png)
...
![Untitled](Neural%20Networks%20If%20I%20only%20had%20a%20brain/Untitled%204.png)

![Untitled](Neural%20Networks%20If%20I%20only%20had%20a%20brain/Untitled%205.png)
...
But the scientists were worried: *had it actually found a way to recognize if there was a tank in the photo, or had it merely memorized which photos had tanks and which did not?*
...
![approximation.png](Neural%20Networks%20If%20I%20only%20had%20a%20brain/approximation.png)
...
![relu.png](Neural%20Networks%20If%20I%20only%20had%20a%20brain/relu.png)
...
![backpropagation.png](Neural%20Networks%20If%20I%20only%20had%20a%20brain/backpropagation.png)
...
- **Long Short-Term Memory (LSTM) and Gated Recurrent Units (GRU):** Variants of RNNs designed to solve the vanishing gradient problem associated with standard RNNs. LSTMs and GRUs introduce gates that regulate the flow of information, allowing the network to retain or forget information over long sequences effectively.
    ...
    ![rnn.png](Neural%20Networks%20If%20I%20only%20had%20a%20brain/rnn.png)
    

![LSTMvsGRU.png](Neural%20Networks%20If%20I%20only%20had%20a%20brain/LSTMvsGRU.png)

![lstmvsgru2.png](Neural%20Networks%20If%20I%20only%20had%20a%20brain/lstmvsgru2.png)
...
![tx_basic.png](Neural%20Networks%20If%20I%20only%20had%20a%20brain/tx_basic.png)

![tx_moderate.png](Neural%20Networks%20If%20I%20only%20had%20a%20brain/tx_moderate.png)

![1_vrSX_Ku3EmGPyqF_E-2_Vg.png](Neural%20Networks%20If%20I%20only%20had%20a%20brain/1_vrSX_Ku3EmGPyqF_E-2_Vg.png)
...
![The left and center figures represent different layers / attention heads. The right figure depicts the same layer/head as the center figure, but with the token *lazy* selected](Neural%20Networks%20If%20I%20only%20had%20a%20brain/attention.png)
...
![simple-pretty-gif.gif](Neural%20Networks%20If%20I%20only%20had%20a%20brain/simple-pretty-gif.gif)
...
<aside>
💡 **NOTE:** [Keras documentation for detailed function definitions](https://www.tensorflow.org/guide/keras)

</aside>

**Model Structure:**
Begin by initializing a `Sequential` model in Keras, then sequentially add layers, starting from input to output. For the EMNIST dataset, a simple model might include a layer to reshape the input, a flattening layer to convert 2D images to 1D vectors, and dense layers for classification purposes:
...
After defining the model, compile it with the chosen optimizer and loss function, and then train it using the `fit` method, specifying your training data, batch size, and epochs:
...
<aside>
💡 **NOTE:** [PyTorch documentation for detailed function definitions](https://pytorch.org/docs/stable/index.html)

</aside>

**Custom Model Definition:**
In PyTorch, define a custom neural network by subclassing `nn.Module`. Initialize the layers in the constructor and specify the forward pass logic. A straightforward network might include a fully connected layer for the hidden layer and an output layer:
...
Choose a suitable loss function and optimizer. For classification, `CrossEntropyLoss` is typically used, and `Adam` is a widely used optimizer:
...
In PyTorch, iterate over your dataset in batches, pass the inputs through the model to obtain outputs, compute the loss, and update the model parameters using `loss.backward()` and `optimizer.step()`:
...
**Keras Training:**
In Keras, training the model is straightforward with the `fit` method, where `x_train` and `y_train` are your training inputs and labels:
...
- **Hybrid Models:** Demonstrating the combination of CNNs and RNNs in Keras for tasks that require understanding both spatial and temporal data.
    - https://github.com/ankangd/HybridCovidLUS: An example of a hybrid model applied to medical imaging for COVID-19 analysis.
- **GPT from Scratch:** Building a simplified version of the GPT model in PyTorch, offering insights into the workings of transformer models.
    - https://github.com/karpathy/nanoGPT: Accompanied by a [full walkthrough video](https://www.youtube.com/watch?v=kCc8FmEb1nY), this implementation demystifies the GPT architecture.
- **Multi-Model Systems:** Exploring the interaction between different models, such as in GANs, where a generative model is pitted against a discriminative model to produce high-quality synthetic data.
    - https://github.com/tezansahu/PyTorch-GANs: A beginner-friendly implementation showing how to train a GAN on the MNIST dataset to generate digit images.
...
[https://github.com/TheEconomist/the-economist-war-fire-model](https://github.com/TheEconomist/the-economist-war-fire-model)
...
- *What are embeddings?,* Vicki Boykis [available at the author’s website](https://vickiboykis.com/what_are_embeddings/)

[LLM survey.pdf](Neural%20Networks%20If%20I%20only%20had%20a%20brain/LLM_survey.pdf)

## Books

### Recommendations

- *Python for Data Analysis*, McKinney - author’s [website](https://wesmckinney.com/book/)
- *Python Data Science Handbook,* VanderPlas - author’s [website](https://jakevdp.github.io/PythonDataScienceHandbook/)
- *PyTorch Tutorials* - official [documentation](https://pytorch.org/tutorials/beginner/deep_learning_60min_blitz.html)
- *TensorFlow Tutorials* - official [documentation](https://www.tensorflow.org/tutorials)
- *Dive into Deep Learning -* authors’ [website](https://d2l.ai)
- *Understanding Deep Learning* ****- author’s [website](https://udlbook.github.io/udlbook/) (**WARNING:** intense math)
...
- https://github.com/microsoft/RespireNet - A CNN-based model designed for COVID-19 severity prediction from lung ultrasound images, showcasing the application of neural networks in healthcare.
- https://github.com/ritchieng/the-incredible-pytorch - curated list of tutorials, projects, libraries, videos, papers, and books